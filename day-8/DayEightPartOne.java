...
public class DayEightPartOne {
	...
	public static void main(String args[]) {
		...
		try (FileReader fr = new FileReader("./sampleInput"); BufferedReader br = new BufferedReader(fr)) {
	...
	public static Set<AntinodeLocation> getPossibleAntinodeLocationsFromAntennas(AntennaLocation a1, AntennaLocation a2, List<String> lines) {
		...
		// check if both antennas are in bounds
		Set<AntinodeLocation> locationOfAntinodes = getAntinodeLocationsFromAntennas(a1, a2, lines);
		for (AntinodeLocation location : locationOfAntinodes) {
			System.out.println(location);
			if (isLocationInBounds(location, lines)) {
	...
	public static Set<AntinodeLocation> getAntinodeLocationsFromAntennas(AntennaLocation a1, AntennaLocation a2, List<String> lines) {
		...
		locations.add(new AntinodeLocation(a1.lineIndex + verticalDifference, a1.characterIndex + horizontalDifference));  // keeping the original sign
		locations.add(new AntinodeLocation(a2.lineIndex - verticalDifference, a2.characterIndex - horizontalDifference));  // reversing the sign
